...
## Where plugins, skills, hooks, and dotfiles are stored:
...
- Zod schemas at boundaries only — app edges and module contracts, not inside
  modules. Derive types with `z.infer<typeof Schema>`
...
- Modular monolith → vertical slices → evolve to CQRS/microservices when
  complexity demands
