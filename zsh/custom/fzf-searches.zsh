...
function nt() {
  local dir_list="$(find $NOTES -type f -regex ".*.md$" -mindepth 1 -maxdepth 1 | sort -r | fzf --preview "bat --color=always --style=numbers {}" \
...
function videos() {
  ...
  local dir_list="$(find "${DIRS_PATHS[@]}" -mindepth 1 -maxdepth 2 -type d | sort -u)"
