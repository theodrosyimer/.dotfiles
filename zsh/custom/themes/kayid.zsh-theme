...
# > another arrow for the prompt
PROMPT+="%(?:%{$fg_bold[green]%}❯ :%{$fg_bold[red]%}❯ )"
GIT_PROMPT='$(out=$(git_prompt_info);if [[ -n $out ]]; then printf %s "$purple$out$white [$(git_prompt_status)$(git_remote_status)$white] $reset";fi)'
PROMPT+="$GIT_PROMPT"

# PROMPT+='$nl$orange$%{$reset_color%} '
# RPROMPT="$GIT_PROMPT"

RPROMPT+="%{$reset_color%}${return_status}$(git_time_since_commit)%{$reset_color%}"
